...
def rebalance(
    ...
    needs_rebalance = (
        abs(usdc_percent_allocation - TARGET_ALLOCATIONS["usdc"]) > BUFFER or
        abs(weth_percent_allocation - TARGET_ALLOCATIONS["weth"]) > BUFFER
    )
    ...
    if needs_rebalance:
        ...
        trades_tokens = calculate_rebalancing_trades(
            {"balance": a_usdc_balance_normalized, "price": get_price("usdc_usd_price_feed"), "contract": usdc},
            {"balance": a_weth_balance_normalized, "price": get_price("eth_usd_price_feed"), "contract": weth},
            TARGET_ALLOCATIONS
        )
        ...
        trades_token_to_buy = trades_tokens["usdc"] if trades_tokens["usdc"]["trade"] > 0 else trades_tokens["weth"]
        trades_token_to_sell = trades_tokens["usdc"] if trades_tokens["usdc"]["trade"] <= 0 else trades_tokens["weth"]
